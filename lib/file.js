...
FilePersistence.prototype.persistImpl = function () {
    ...
    if ((!self.unpersistRequested) && (!self.removeListeners)) {
...
FilePersistence.prototype.writeAlways = function (snapshot) {
    ...
    if (snapshot) {
        if (self.inWrite) {
            ...
            self.inWrite = true;
            fs.writeFile(this.filePath, JSON.stringify(snapshot, undefined, self.options.prettyPrint ? 2 : 0), function (error) {
                self.inWrite = false;
