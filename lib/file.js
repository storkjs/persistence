 ...
 */
function FilePersistence(merger, options, callback) {
...
FilePersistence.prototype.setupFileSystem = function (callback) {
    var self = this;
    persistenceUtils.createDirectoryIfNeeded(function (error) {
        if (error) {
            debug('File persistence, unable to create directory, error: ' + error.message, error);
            self.emit('create-error', error);
        }

        persistenceUtils.createFileIfNeeded(function (error2) {
            if (error2) {
